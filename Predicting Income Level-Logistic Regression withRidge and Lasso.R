...
income_data_imputed_tarfac <- income_data_imputed

income_data_imputed_tarfac$target_variable <- as.factor(income_data_imputed_tarfac$target_variable)
# 
# target <- c("target_variable")
# 
# y<- data.frame(as.factor(income_data_imputed[,target]))
# 
# names(y) <- "target"
# 
# x <- income_data_imputed[,!names(income_data_imputed) %in% target]

grid_params <- data.frame(expand.grid(alpha = c(0, 0.5, 1),                                 lambda = c(0.2, 0.3, 0.4)))

val_strategy <- trainControl(method = "repeatedcv", 
                             number = 10,
                             repeats = 5)

rid_las_model <- train(form = target_variable~.,data =  income_data_imputed_tarfac,method = "glmnet", 
      metric = "Accuracy",
      trControl = val_strategy, 
      tuneGrid = grid_params)


#Predict the values on Train data

prob_train <- predict(rid_las_model,type="raw")

head(prob_train,10)

coef(rid_las_model$finalModel$)

#Create a Prediction object using ROCR

library(ROCR)

pred <- prediction(prob_train,income_data_imputed_tarfac$target)
...
prob_test <- predict(rid_las_model,test_data_imputed,type = "raw")
...
final_data <-cbind(index,test_data_imputed,prob_test)
...
write.csv(submission, "C:/Users/Yunus Saleem/Desktop/Insofe/Cute/Submission-ridge-lasso.csv")
