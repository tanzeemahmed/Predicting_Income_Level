...
income_data_imputed$tax_paid <- ifelse(income_data_imputed$tax_paid <6200 ,"Low Tax Paid",ifelse(income_data_imputed$tax_paid>6200,"High tax paid","No tax paid"))

income_data_imputed$tax_paid[is.na(income_data$tax_paid)] <- "No Tax Paid"

income_data_imputed$tax_paid <- as.factor(income_data$tax_paid)
...
##############Backup of main datasets

income_backup <- income_data_imputed

test_backup <- test_data_imputed
...
train_rows <- createDataPartition(income_data_imputed$age,p=0.7,list=F)

income_data_imputed <- income_backup[train_rows,]

val_data_imputed <- income_backup[-train_rows,]

val_data_imputed$target_variable <- NULL

val_data_imputed$white <- NULL

#Read test data and convert the variables appropriately

test_data <- read.csv("test_data.csv",header = TRUE,strip.white = TRUE)
...
colSums(is.na(num_attr_data_test_imputed))
...
log_reg <- glm(target_variable~.-loan_taken-tax_paid, data = income_data_imputed, family = binomial)
...
#Choose cutoff at 0.1 and predict on validation data

plot(perf,col=rainbow(10), colorize = T,print.cutoffs.at=0.1)

prob_test <- predict(log_reg,val_data_imputed,type = "response")
...
val <- as.character(val_data_imputed$target_variable)

confusionMatrix(data = preds_test,reference = val,positive = "1")
...
write.csv(submission, "C:/Users/Yunus Saleem/Desktop/Insofe/Cute/Submission-remtaxloa.csv")
